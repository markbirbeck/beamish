  ...
  async _read() {
    ...
    if (this.records === null) {
      ...
      if (!this.config.size || (this.recordsRead < this.config.size)) {
        ...
        /**
         * ...if a path to a cursor has been specified then use it
         * to grab the cursor, which we'll use to signify that more
         * reading is needed:
         */
        if (this.config.cursorPath) {

          /**
           * TODO(MB): Remove eval() in favour of just navigating through
           * the objects:
           */
          eval(`this.cursor = res.data.${this.config.cursorPath}`)

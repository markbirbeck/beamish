  ...
  async processBundle() {
    ...
    const graph = g.map((transform, index) => {
      ...
      if (index === 0) {
        ...
        if (transform.doFn && (transform.doFn.stream instanceof stream.Readable || transform.doFn.stream.constructor.name === 'Readable')) {
      ...
      if (index === g.length - 1) {
        ...
        if (transform.doFn && (transform.doFn.stream instanceof stream.Writable)) {
      ...
      if (transform.doFn && (transform.doFn.stream instanceof stream.Duplex)) {
        return new DoFnAsDuplex(transform)
      }

      if (transform.doFn && (transform.doFn.stream instanceof stream.Transform) ||
        transform.doFn.stream === undefined) {
